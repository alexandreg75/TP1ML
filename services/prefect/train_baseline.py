...
def fetch_labels(engine, as_of):
    # si ton schéma labels a period_start, on l’utilise ; sinon fallback.
    try:
        q = text("""
            SELECT user_id, period_start, churn_label
            FROM labels
            WHERE period_start = :as_of
        """)
...
def main():
    ...
    # TODO 7: MLflow run + train + metrics + logs
    with mlflow.start_run(run_name=f"rf_baseline_{AS_OF}") as run:
        ...
        # TODO 9: log_model -> on log la pipeline complète (prep + modèle)
