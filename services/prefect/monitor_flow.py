...
def compute_target_drift(reference_df: pd.DataFrame, current_df: pd.DataFrame) -> float:
    """
    Drift simple sur la cible : différence absolue du taux de churn entre périodes.
    """
...
def run_evidently(reference_df: pd.DataFrame, current_df: pd.DataFrame, as_of_ref: str, as_of_cur: str):
    ...
    # Seuil “arbitraire” pour ce TP : part de colonnes driftées
    DRIFT_SHARE_THRESHOLD = 0.3

    # Colonne à monitorer avec ValueDrift : churn_label si dispo, sinon fallback
    target_col = "churn_label" if "churn_label" in reference_df.columns and "churn_label" in current_df.columns else "monthly_fee"
...
def monitor_month_flow(
    ...
    print(
        ...
        f"drift_share={res['drift_share']:.2f} value_drift_col={res['value_drift_column']} -> {msg}"
    )
