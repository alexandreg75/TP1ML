...
def _to_bool_series(s: pd.Series) -> pd.Series:
    """
    Convertit proprement une colonne CSV (souvent 'True'/'False', '1'/'0', etc.)
    vers un booléen pandas nullable ('boolean').
    """
    ...
    # Si certaines valeurs sont déjà True/False ou vides, on garde nullable
...
def upsert_csv(table: str, csv_path: str, pk_cols: list[str]):
    ...
    # TODO (1) : convertir en booléen les colonnes booléennes si elles existent
    ...
    with eng.begin() as conn:
        ...
        # Table temporaire (schéma basé sur df -> OK si dtypes corrects)
        ...
        # TODO (2) : construire "col = EXCLUDED.col" pour toutes les colonnes non PK
...
def validate_with_ge(table: str):
    ...
    if table == "users":
        ...
        # Vérification stricte du schéma
        gdf.expect_table_columns_to_match_set([
            "user_id",
            "watch_hours_30d",
            "avg_session_mins_7d",
        ...
        # Bornes physiques : ces métriques ne peuvent pas être négatives
